                ...
                {showCatMenu && (
                    <ul className="bg-black/[0.05] -mx-5 mt-4 -mb-4">
                        {subMenuData.map((submenu) => {
                            return (
                                <Link href="/store/category" key={submenu.id} onClick={()=> {setShowCatMenu(false); setMobileMenu(false);}}>
                    ...
                    <li className='px-5 py-4 border-b'>
                        <Link href={item?.url} className='cursor-pointer' onClick={()=> setMobileMenu(false)}>{item.name}</Link>
                    </li>
                )}
            </React.Fragment>
        )
